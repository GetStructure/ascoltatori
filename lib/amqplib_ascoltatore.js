...
AMQPLibAscoltatore.prototype.publish = function publish(topic, message, options, done) {
  this._raiseIfClosed();

  debug("new message published to " + topic);

  var headers = options && options.headers || {};
  this._channel.publish(this._opts.exchange, this._pubTopic(topic), new Buffer(String(message)), { headers: headers });
  defer(done);
};
