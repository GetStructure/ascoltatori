 ...
 *  - 'exchangeOpts', options for the exchange;
...
function AMQPAscoltatore(opts) {
  ...
  this._opts.exchangeOpts = this._opts.exchangeOpts || {};
  this._opts.exchangeOpts.confirm = true;
  this._opts.exchangeOpts.type = "topic";
...
/**
 * The client connection descends from AbstractAscoltatore.
 *
 * @api private
 */
...
AMQPAscoltatore.prototype._startConn = function() {
  ...
  if (this._client_conn === undefined) {
    ...
    steed.series([
      ...
      function(callback) {
        debug("connected");
        that._exchange = conn.exchange(that._opts.exchange, that._opts.exchangeOpts);
      ...
      function(callback) {
        that._queue.subscribe({
          ...
          if(!Buffer.isBuffer(payload)){ payload = String(payload); }
...
AMQPAscoltatore.prototype.publish = function publish(topic, message, options, done) {
  ...
  if(!Buffer.isBuffer(message)){ message = String(message); }
  this._exchange.publish(this._pubTopic(topic), message, { headers: headers });
  defer(done);
};
